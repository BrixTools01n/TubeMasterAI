...
(function () {
  ...
  /**
   * Update User Account Profile Card
   */
  ...
  /**
   * Authenticate with Google ID Token or Credentials
   */
  ...
  /**
   * Check if active session exists on load
   */
  ...
  if (btnGoogleAuthDirect) {
    btnGoogleAuthDirect.addEventListener('click', async () => {
      // In web preview environments, trigger Google Sign-In prompt or verified account chooser
      if (window.google && window.google.accounts && window.google.accounts.id) {
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // Fallback to direct verified creator auth
