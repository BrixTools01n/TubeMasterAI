...
// ============================================================================
// Stores pending verification requests with TTL and attempt limits
const pendingRequests = new Map();
// Stores verified deletion audit completion records
...
/**
 * ============================================================================
 * ACCOUNT DELETION ARCHITECTURE & VERIFICATION BACKEND
 * ============================================================================
 */
...
async function verifyAndExecuteDeletion({ requestId, code, googleIdToken, confirmed }) {
  ...
  // Purge from Pending Store
  pendingRequests.delete(requestId);

  // Record in Completed Deletions Audit Register
...
const server = http.createServer(async (req, res) => {
  ...
  // 1. Health Probe (Google Cloud Run liveness check)
      ...
      try {
        ...
        if (!email || typeof email !== 'string') {
          return sendJson(res, 400, {
            success: false,
            error: 'INVALID_EMAIL',
            message: 'A registered account email address is required.'
          });
        }
        ...
        if (!emailRegex.test(sanitizedEmail) || sanitizedEmail.length > 254) {
          return sendJson(res, 400, {
            ...
            message: 'Please provide a valid, well-formed email address.'
          });
        }
        ...
        if (confirmed !== true && confirmed !== 'true') {
          return sendJson(res, 400, {
            ...
            message: 'You must confirm that you understand account deletion permanently removes your account data.'
          });
        }
        ...
        return sendJson(res, 500, {
          success: false,
          error: 'SERVER_ERROR',
          message: 'An unexpected error occurred while initiating your deletion request. Please try again later.'
        });
      }
    });
    return;
      ...
      try {
        ...
        if (!code && !googleIdToken) {
          return sendJson(res, 400, {
            ...
            message: 'Verification code or Google identity token is required.'
          });
        }
        ...
        if (confirmed !== true && confirmed !== 'true') {
          return sendJson(res, 400, {
            success: false,
            error: 'CONFIRMATION_REQUIRED',
            message: 'You must confirm final deletion.'
          });
        }
        ...
        return sendJson(res, 500, {
          success: false,
          error: 'SERVER_ERROR',
          message: 'An unexpected error occurred during account verification and deletion.'
        });
      }
    });
    return;
  ...
  // 7. Serve Static Files (CSS, JS, Icons)
  if (method === 'GET') {
  ...
  return sendJson(res, 405, { error: 'METHOD_NOT_ALLOWED', message: `Method ${method} not allowed on ${pathname}` });
});
...
if (require.main === module) {
  const runningServer = server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(` Request Deletion: POST /api/account-deletion/request`);
    console.log(` Verify & Delete:  POST /api/account-deletion/verify`);
    console.log(` Support Email: ${CONFIG.SUPPORT_EMAIL}`);
    console.log(` Privacy Policy: ${CONFIG.PRIVACY_POLICY_URL}`);
    console.log(`====================================================`);
  });
